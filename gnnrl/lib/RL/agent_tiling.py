...
class ActorNetworkTiling(nn.Module):
    ...
    def forward(self, state):
        ...
        actions = self.sigmoid(self.linear2(actions))
        return actions
...
class CriticNetworkTiling(nn.Module):
    def __init__(self, g_in_size, g_hidden_size, g_embedding_size,
                 chkpt_dir='tmp/rl'):
        super(CriticNetworkTiling, self).__init__()
    ...
    def forward(self, state):
        ...
        value = self.sigmoid(self.linear1(g))
        return value
...
class ActorCriticTiling(nn.Module):
    def __init__(self, state_dim, action_dim, action_std):
        super(ActorCriticTiling, self).__init__()
        ...
        self.actor = ActorNetworkTiling(**actor_cfg)
        self.critic = CriticNetworkTiling(**critic_cfg)
...
class AgentTiling:
    def __init__(self, state_dim, action_dim, action_std, lr, betas, gamma, K_epochs, eps_clip):
        ...
        self.policy = ActorCriticTiling(state_dim, action_dim, action_std).to(device)
        # self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=lr, betas=betas)
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.policy.parameters()), lr=lr, betas=betas)

        self.policy_old = ActorCriticTiling(state_dim, action_dim, action_std).to(device)
